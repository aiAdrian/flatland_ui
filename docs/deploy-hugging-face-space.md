# The demo Space, and updating it from a browser

The stable demo lives on a Hugging Face **Docker Space** built from `main`.
`.github/workflows/deploy-hf-space.yml` pushes it there on every push to
`main`, so the demo can be refreshed without a terminal — from Claude Code on
the web, the GitHub web editor, or a phone on a train.
...
out of `main`) and a `Dockerfile` whose runtime user is UID 1000, because that
...
Then every push to `main` redeploys. `workflow_dispatch` re-runs it by hand.
